...
- チップの形式: `🔧 Skill(skill-name) <絶対パス>`。パスは html では `file://` リンク、md ではインラインコード表記
...
- エージェント発動: `<div class="skill">🔧 Skill(<b>名前</b>) <a href="{{fileURL .Skill.Path}}">パス</a></div>`
  を発言バブルの外に置く
...
`file://` URL はテンプレート内の文字列連結で組み立てず、render 層に追加する
`fileURL` 関数で生成する。

- `url.URL{Scheme: "file", Path: path}` で構築し、`template.URL` として返す。
  これによりパス中の空白・`#`・`?`・非ASCII が正しく %エスケープされる (テンプレート内のリテラル連結では `#` `?` がエスケープされずリンクが壊れる)
- `template.URL` への変換は html/template の安全機構の意図的な迂回なので、汎用のキャスト関数にはせず、file スキーム + 絶対パス専用の関数とする
- 入力パスが絶対パスでない場合は空文字列を返し、テンプレート側でリンクにせずテキスト表示へ縮退する (レンダリングエラーで出力全体を止めない)
...
- html: `.skill` チップ・`file://` リンク・ユーザーバブルへの埋め込み
- html: 生成された href が期待どおりの `file://` URL であること (`#ZgotmplZ` でないこと)。空白・`#` を含むパスが %エスケープされること
- html: 相対パス入力でリンクにならずテキスト表示へ縮退すること
